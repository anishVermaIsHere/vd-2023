...
const Login = () => {
        ...
        const response=axios.post('/api/login', user)
          .then(function (response) {
            if(response.status==200){
                setMessage({text:`token:${response.data.token}`,color:"success", type:"Success"});
                setToast(true);
                setTimeout(()=>{
                    setToast(false);
                },5000)
          ...
          .catch(function (error) {
            ...
            if(error.response.status==400) {
                setMessage({text:`${error.response.data.error}`,color:"danger", type:"Error"});
                setToast(true);
            }
          });
    ...
    return (
        <div className="login-layout w-100 m-auto">
            <form onSubmit={handleSubmit} method='POST'>
                <img className="mb-4" src="https://upload.wikimedia.org/wikipedia/commons/e/ee/Sample_abc.jpg" alt width={220} height={103} />
                {toast&&<Toast message={message.text} color={message.color} type={message.type}/>}
                <div className="form-container">
                    <div className="login-label-section">
                        <h1 className="welcome h3 text-center">Welcome</h1>
                        <p className="label-text text-14-400 text-center">Enter your Username and Password</p>
                    </div>
                    <div className="form-floating">
                        <span className="input-icon">
                            <svg width="16" height="17" viewBox="0 0 16 17" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <g clip-path="url(#clip0_7113_1426)">
                                ...
                                <defs>
                                    <clipPath id="clip0_7113_1426">
                                        <rect width="16" height="16" fill="white" transform="translate(0 0.29895)" />
                                    </clipPath>
                                </defs>
                            </svg>
                        </span>
                        <div className="form-input">
                            <input type="email"
                                ...
                                className="" id="email" placeholder="Username" />
                        </div>
                    </div>
                    <div className="form-floating">
                        <span className="input-icon">
                        ...
                        <span className="input-icon show-password">
                    ...
                    <div className="form-btn">
                        <button className="w-100 login-btn" type="submit">Login</button>
                        <p className="mb-0 forget-password text-center">
                ...
                <div className="login-footer">
                    ...
                    <p className="mb-3 text-muted text-12-400">© Punctualiti {new Date().getFullYear()}. All rights reserved</p>
                </div>
            </form>
